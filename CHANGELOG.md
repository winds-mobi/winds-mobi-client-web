...
- **🧪 Beta:** Clicking or tapping the map can now close the open station panel, so you no longer have to aim for the close button. Clicking another station still switches straight to it, and panning the map leaves the panel open. Off by default; turn it on under Settings → Beta features. ([#157](https://github.com/winds-mobi/winds-mobi-client-web/issues/157))
...
- Upgraded the underlying map library (MapLibre GL JS) to its latest major version.
  - _aka: we swapped some engine parts under the map. Should feel exactly the same to drive — if it doesn't, that's a bug, not a feature._
- Upgraded the app's build tooling — Vite and TypeScript — to their latest major versions, and switched to Ember's current official application blueprint.
  - _aka: the developers moved furniture around in a room you'll never see. Nothing here should affect you at all, which is the whole point — and also exactly the kind of change most likely to sneak a bug past us._
...
- The Help page now shows which version of the app you're running, with a link to the full changelog for that release on GitHub.
