...
- Prefer declarative rendering state over imperative UI bookkeeping. Use class getters for derived data, and treat excessive local `@tracked` state as a sign that there is probably a simpler root state to derive from.
...
- Never invent manual loading or pending booleans such as `@tracked isLoading`, `@tracked isPending`, `@tracked loading`, or similar flags in app UI code. Prefer encapsulated resource state from Ember, Warp Drive `<Request>`, or other addon APIs so loading, error, and data stay synchronized, and avoid writing temporal state back after an `await` just to mirror request lifecycle.
